...
## 脚本设置
设置主页运行任务的具体参数，可展开折叠，支持添加移除配置，右键可以重命名配置，支持多配置切换，切换后会结束当前任务并断开连接
...
:::tip
不想频繁修改配置可以创建多个常用配置，例如日常一个、活动一个，新建的配置会复制当前的配置，模拟器和区服没有改变只需要更改脚本设置即可
:::
## 日志窗口
任务结束后会输出战斗结果(用时、战斗数、掉落数)，以及结束的原因，出现问题时如果要反馈，请将完整的当前游戏画面和log截图反馈