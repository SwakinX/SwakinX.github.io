...
## 基本功能
可用左右切换当前显示的功能，使用前请先在[`脚本设置`](#脚本设置)里设置好战斗次数，礼装类型，是否要清理剩余体力，使用茶壶等
### 日常任务
...
自动刷各种本，主要是能连续战斗的，或是强化本(更推荐使用[`首通关卡`](#首通关卡))或者free本那样回到主界面右上角也有本可以点的情况。复杂的机制关可能不能正常通关，建议手动修改([Alter界面说明](../mooncancer2))根据机制来完成战斗。可以在进入战斗前的选择助战，队伍设置和进入战斗后开始，也可以在主界面选本列表开始，但要保证要刷的本在第一个位置。<br>
...
## 脚本设置
设置主页运行任务的具体参数，可展开折叠，支持添加移除配置，右键可以重命名配置，支持多配置切换。每个配置都可以独立运行任务（多开互不干扰），切换标签只会切换界面显示的配置，不会结束正在运行的任务
...
- **使用茶壶** 在编队界面和继续战斗时检查茶壶使用状态（包括风暴罐）
- **苹果和助战** 选择补充体力的苹果类型，选择助战礼装，支持冠位助战，具体请阅读[礼装规则](../mooncancer1#礼装规则)
- **活动点限制** 设置活动点限制，超过就停止，不多刷浪费，不限制时设为0，没有活动点的情况不触发
- **抽每日免费** 开启会在日常任务打白纸化之前先抽取每日免费（友情池和剧情免费召唤）
- **日常任务** 选择每日刷的本，可选择白纸化地球本和冠位戴冠战，或者都不打，按需选取，但要注意是否已经开放。冠位战的情况下要把冠位认定战打了，因为默认打最后一个本，不过没打的话会自动打了，要注意一下

### 礼装规则
- 礼装最多可以候选四种，检测到其中一个就会选取
- 当包含活动礼装时，会默认选取第一个助战，也就是不挑助战，想要选活动礼装设置好筛选
- 当检测到冠位助战时，会选取满足候选列表其中两项的助战
:::tip
不想频繁修改配置可以创建多个常用配置，例如日常一个、活动一个，新建的配置会复制当前的配置，模拟器和区服没有改变只需要更改脚本设置即可<br>
多个配置的任务可以同时运行（多开），比如一个配置挂日常、另一个配置刷活动；注意同一个模拟器同时只能被一个配置占用，多开请使用多个模拟器实例
:::
## 日志窗口
任务结束后会输出战斗结果(用时、战斗数、掉落数)，以及结束的原因，出现问题时如果要反馈，请将完整的当前游戏画面和log截图反馈<br>
多开时主页日志会以不同颜色的`[配置名]`前缀区分来源，logs文件夹下每个配置还有独立的日志文件，方便分开排查